...
public class DrawingSurface extends PApplet {
		...
		new Thread() {
			public void run() {
				predictor = new NetFlixPredictor(moviesFile,ratingsFile,tagsFile,linksFile);
				 ...
				 int thisMovie = Collections.binarySearch(predictor.getMovies(), new Movie(recommendedID));

				 Movie m = predictor.getMovies().get(thisMovie);
	...
	public void draw() {
		...
		if (predictorLoaded) {
			...
			textAlign(LEFT);
			textSize(20);
			text("All Movies", 10,  movieHeight/4-15);
			...
			textAlign(LEFT);
			textSize(30);
			text("Recommended", DRAWING_WIDTH/2-DRAWING_WIDTH/4,  DRAWING_HEIGHT/2.5f-40);
